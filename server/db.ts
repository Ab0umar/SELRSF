...
import { InsertUser, users, employees, transactions, InsertEmployee, InsertTransaction } from "../drizzle/schema";
...
export async function getTransactionsByYear(year: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(transactions).where(eq(transactions.year, year));
}
