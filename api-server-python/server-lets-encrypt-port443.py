"""
SELRS API Server V14 - Port 443 (HTTPS) for Cloudflare
"""
...
# Configuration
SECRET_KEY = "selrs2024_secure_and_long_secret_key_for_jwt_compliance_v14"
ALGORITHM = "HS256"
...
def get_bait(user):
    try:
        ...
        for row in cursor.fetchall():
            ...
            payment = record.get('منه', 0) if record.get('منه') else 0
...
def health_check():
    ...
    return jsonify({
        'status': 'ok',
        'message': 'SELRS API Server V14 is running on port 443',
...
if __name__ == '__main__':
    ...
    print("\n" + "="*60, flush=True)
    print("SELRS API Server V14 - Cloudflare HTTPS (Port 443)", flush=True)
    print("="*60, flush=True)
    ...
    print("\n" + "="*60, flush=True)
    print("🔒 Starting HTTPS server on port 443...", flush=True)
    print("="*60, flush=True)
    print(f"📍 Local:   https://127.0.0.1", flush=True)
    print(f"🏠 Network: https://{local_ip}", flush=True)
    print(f"🌐 Domain:  https://selrs.cc", flush=True)
    print(f"✅ Health:  https://selrs.cc/api/health", flush=True)
    print(f"📱 Mobile:  Use 'https://selrs.cc' in app settings", flush=True)
    print("="*60 + "\n", flush=True)
