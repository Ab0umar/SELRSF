...
import { parseInstapayExcel } from "@/lib/excel-import";
...
const SYNC_FILE_KEY = "instapay_sync_file";
const AUTO_SYNC_KEY = "instapay_auto_sync_enabled";

export default function InstapayScreen() {
  const { showToast } = useToast();
  const [items, setItems] = useState<InstapayItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState("");

  // Form state
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [editingItem, setEditingItem] = useState<InstapayItem | null>(null);
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const data = await getInstapayItems();
      setItems(data);
    } catch (error) {
  ...
  const autoImportFromFile = async (filePath: string) => {
    try {
      const rows = await parseInstapayExcel(filePath);
      
      if (rows.length === 0) return;
      
      for (const row of rows) {
        await createInstapayItem({
          name: row.name,
          date: row.date,
  ...
  const openEditModal = (item: InstapayItem) => {
    setEditingItem(item);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingItem) {
        await updateInstapayItem(editingItem.id, data);
      } else {
        await createInstapayItem(data);
      }

      // Add to server sync queue
      await serverSyncService.addPendingItem("instapay", data);
  ...
  const handleDelete = (item: InstapayItem) => {
    Alert.alert("تأكيد الحذف", `هل تريد حذف ${item.name}؟`, [
      { text: "إلغاء", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await deleteInstapayItem(item.id);
            await loadData();
          } catch (error) {
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
        ...
        <View className="flex-row items-center justify-between mb-4">
          ...
          <View className="items-center flex-1">
            ...
            <Text className="text-muted">انستا</Text>
          </View>
