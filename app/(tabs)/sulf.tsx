...
import { generateSulfPDF } from "@/lib/pdf-generator";
import { parseSulfExcel } from "@/lib/excel-import";
import {
  getSulfItems,
  createSulfItem,
  updateSulfItem,
...
export default function SulfScreen() {
  ...
  const [items, setItems] = useState<SulfItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState("");

  // Form state
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [editingItem, setEditingItem] = useState<SulfItem | null>(null);
  ...
  const autoImportFromFile = async (filePath: string) => {
    try {
      const rows = await parseSulfExcel(filePath);
      
      if (rows.length === 0) return;
      
      for (const row of rows) {
        await createSulfItem({
          name: row.name,
          date: row.date,
          advance: row.advance,
          payment: row.payment,
  ...
  const importFromFile = async () => {
    ...
    setIsImporting(true);
    try {
      const rows = await parseSulfExcel(syncFilePath);
  ...
  const openEditModal = (item: SulfItem) => {
    setEditingItem(item);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingItem) {
        await updateSulfItem(editingItem.id, data);
      } else {
        await createSulfItem(data);
      }

      // Add to server sync queue
      await serverSyncService.addPendingItem("sulf", data);
  ...
  const handleDelete = (item: SulfItem) => {
    Alert.alert("تأكيد الحذف", `هل تريد حذف سلفة ${item.name}؟`, [
      { text: "إلغاء", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await deleteSulfItem(item.id);
            await loadData();
          } catch (error) {
            Alert.alert("خطأ", "فشل في حذف السلفة");
          }
        },
      },
    ]);
  ...
  const totalLoans = filteredItems.reduce((sum, item) => sum + item.advance, 0);
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
        ...
        <View className="flex-row gap-2 mb-4">
          <View className="flex-1 bg-surface p-3 rounded-lg border-l-4 border-warning">
            <Text className="text-xs text-muted mb-1">إجمالي السلف</Text>
        ...
        <View className="flex-row gap-2 mb-4">
          ...
          >
            <Text className="text-center text-white font-bold">+ إضافة سلفة</Text>
          </TouchableOpacity>
          <TouchableOpacity
        ...
        <Text className="text-center text-muted mb-4">
          عدد السلف: {filteredItems.length}
        ...
        {isLoading ? (
          ...
          <Text className="text-center text-muted">
            {searchQuery ? "لا توجد نتائج للبحث" : "لا توجد سلف"}
          </Text>
        ) : (
          <FlatList
            data={filteredItems}
            ...
            renderItem={({ item }) => {
              const remaining = item.advance - item.payment;
              return (
                <View className="bg-surface p-4 rounded-lg mb-2">
                  ...
                  <View className="flex-row justify-between">
                    <Text className="text-warning">
                      سلفة: {item.advance.toLocaleString()}
                    </Text>
                    <Text className="text-success">
                      مسدد: {item.payment.toLocaleString()}
                    </Text>
        ...
        <View className="flex-1 justify-end bg-black/50">
          <View className="bg-background rounded-t-3xl p-6">
            <Text className="text-2xl font-bold text-foreground mb-4">
              {editingItem ? "تعديل سلفة" : "إضافة سلفة"}
            </Text>
